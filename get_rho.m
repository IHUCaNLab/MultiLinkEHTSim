function [rho, rx_p] = get_rho(tx_p,x_a,x_b,y_a,y_b,z_a,z_b,ant_g_a,ant_g_b, fc, shadowing, walls, c)
%GET_RHO   Computes rho (signal-to-noise ratio). Rho is returned in a 
%linear form. Returns also the received power RX_P in dBm.

% [rho, rx_p] = GET_RHO(tx_p, x_a, x_b, y_a, y_b, z_a, z_b, ant_g_a, ant_g_b, fc, shadowing, walls, c)
%       where:
...
% https://www.gnu.org/licenses/gpl-3.0.html
    ...
    % noise power computed at 20 MHz with a noise factor equal to 7 (average).
    noise_power = -174 + 10*log10(20e6) + 7;

    tx_p_lin = 10 ^ (tx_p / 10);
    tx_p_split = tx_p_lin / c.spatial_streams;
    tx_p_split_dbm = 10 * log10(tx_p_split);
    rx_p = tx_p_split_dbm - pl + ant_g_a + ant_g_b - shadowing;
    rho_dbm = tx_p_split_dbm - pl + ant_g_a + ant_g_b - noise_power - shadowing;
    rho = 10^(rho_dbm/10);
